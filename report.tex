...
\author{Shabarish431 \\ \& \\ KarthikPV1198}
...
\section{Usage}
\begin{itemize}
    \item In the terminal run \texttt{bash main.sh}, you will get this output: \\ \newline
    \includegraphics[width=\textwidth]{BS.png}
    \item you have to press only 1 or 2 other numbers/alphabets terminate the script
\item If you want to regester give a username and password(there are few conditions to met, you can see them as you try to register) and you can later login, or if you want to login you can directly login with correct username and password combination 
\item But you can not play the games until \textbf{2} users logged in 
\item After 2 players are logged in you will see the following image with a new window opened(closing leads to termination of the entire project) \\ \newline
    \includegraphics[width=\textwidth]{MWS.png}
\item But with the usernames displayed above(in the red region)
\item Select any game to play or select ANALYTICS to view the charts or select LEADERBOARDS to view the leaderboard in the terminal, you can also exit by clicking the exit option(project terminates)
\item You can choose any metric in by selecting the 'Sort By' dropdown to sort the leaderboard
\item Click on any game you will be presened with a pop-up asking if the user with the username mentioned in the pop-up wanted to make first move 
\item At this point you can't exit, you shall play
\item after selecting any option you can see a loading page for the game you selected there you can exit(Quit option)
\item When game starts you can play or quit(if you quit you lose the match)
\item After gameplay you are presented with a figure containing charts, graphs and also leaderboard in the terminal
\item You are asked if you want to continue playing or quit(a pop-up appears) like this \\ \newline
    \includegraphics[width=\textwidth]{EP.png}
\item If you quit all the project terminates, else you will be returned to main-menu

\end{itemize}
\textbf{HAVE FUN PLAYING THE GAMES :)}
...
\end{itemize}
\nocite{*}
\bibliographystyle{plain}  % or ieee, unsrt, alpha, etc.
